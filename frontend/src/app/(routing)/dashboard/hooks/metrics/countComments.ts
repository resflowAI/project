// features/chars/api/useCountCommentsKpi.ts
...
export function useCountCommentsKpi(concurrent?: string) {
  const title = 'Количество собранных отзывов';
  const { period, range, extra } = useFiltersStore().value;
  const swr = useMappedSWR<CountCommentsRes, KpiCardPayload>(
    ['/metrics/count_comments', { params: { period, range, extra, concurrent } }],
    (api) => ({
      id: api.id,
      title,
