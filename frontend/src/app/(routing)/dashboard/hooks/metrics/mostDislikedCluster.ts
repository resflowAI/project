...
export function useMostDislikedClusterKpi(concurrent?: string) {
  const title = 'Наибольшая зона проблем';
  const { period, range, extra } = useFiltersStore().value;
  const swr = useMappedSWR<CountCommentsRes, KpiCardPayload>(
    ['/metrics/most_disliked_cluster', { params: { period, range, extra, concurrent } }],
    (api) => ({
      id: api.id,
      title,
